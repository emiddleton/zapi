...
type Ticket struct {
	Id                  int64                    `json:"id"`                       // yes 	no 	Automatically assigned when creating tickets
	Url                 string                   `json:"url"`                      // yes 	no 	The API url of this ticket
	ExternalId          *string                  `json:"external_id"`              // no 	no 	An id you can use to link Zendesk tickets to local records
	...
	Recipient           *string                  `json:"recipients"`               //	no 	no 	The original recipient e-mail address of the ticket
	RequesterId         int64                    `json:"requester_id"`             // no 	yes 	The user who requested this ticket
	SubmitterId         int64                    `json:"submitter_id"`             // no 	no 	The user who submitted the ticket; The submitter always becomes the author of the first comment on the ticket.
	AssigneeId          int64                    `json:"assignee_id"`              //	no 	no 	What agent is currently assigned to the ticket
	OrganizationId      *int64                   `json:"organization_id"`          // yes 	no 	The organization of the requester
	GroupId             int64                    `json:"group_id"`                 // no 	no 	The group this ticket is assigned to
	CollaboratorIds     []int64                  `json:"collaborator_ids"`         // no 	no 	Who are currently CC'ed on the ticket
	ForumTopicId        *int64                   `json:"forum_topic_id"`           // no 	no 	The topic this ticket originated from, if any
	ProblemId           *int64                   `json:"problem_id"`               // no 	no 	The problem this incident is linked to, if any
	HasIncidents        bool                     `json:"has_incidents"`            // yes 	no 	Is true of this ticket has been marked as a problem, false otherwise
	DueAt               *Date                    `json:"due_at"`                   // no 	no 	If this is a ticket of type "task" it has a due date. Due date format uses ISO 8601 format.
	Tags                []string                 `json:"tags"`                     // no 	no 	The array of tags applied to this ticket
	Via                 ViaObject                `json:"via"`                      // yes 	no 	This object explains how the ticket was created
	CustomFields        []map[string]interface{} `json:"custom_fields"`            // no 	no 	The custom fields of the ticket
	SatisfactionRating  map[string]interface{}   `json:"satisfaction_rating"`      //	yes 	no 	The satisfaction rating of the ticket, if it exists, or the state of satisfaction, 'offered' or 'unoffered'
	SharingAgreementIds []string                 `json:"sharing_agreement_ids"`    // yes 	no 	The ids of the sharing agreements used for this ticket
	FollowupIds         *[]int64                 `json:"followup_ids"`             // yes  	no 	The ids of the followups created from this ticket - only applicable for closed tickets
	...
	BrandId             int64                    `json:"brand_id"`                 // no 	no 	The id of the brand this ticket is associated with - only applicable for enterprise accounts
	CreatedAt           Date                     `json:"created_at"`               // yes 	no 	When this record was created
	UpdatedAt           Date                     `json:"updated_at"`               // yes 	no 	When this record last got updated
}
...
func (ts *Tickets) List() (tickets []Ticket, err error) {
	path := fmt.Sprintf("%s%s", ts.path, "/tickets.json")
	responseBody, err := ts.client.Get(path, nil)
	if err != nil {
		return []Ticket{}, err
	}
	fmt.Printf("%s\n", string(responseBody))
